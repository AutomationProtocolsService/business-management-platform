...
export async function generatePdf(
  ...
  if (type === 'quote') {
    ...
    content = generateQuotePdfContent(data.quote, data.items as QuoteItem[], data.customer);
  } else {
    if (!data.invoice) throw new Error('Invoice data is required');
    
    content = generateInvoicePdfContent(data.invoice, data.items as InvoiceItem[], data.customer);
  }
...
function generateQuotePdfContent(
  ...
  for (const item of items) {
    content += `${item.description}\t${item.quantity}\t$${item.unitPrice.toFixed(2)}\t$${item.total.toFixed(2)}\n`;
  }
  
  content += `---------------------------------------------------\n`;
  content += `Subtotal:\t\t\t$${quote.subtotal.toFixed(2)}\n`;
  
  if (quote.discount) {
    content += `Discount:\t\t\t$${quote.discount.toFixed(2)}\n`;
  }
  
  if (quote.tax) {
    content += `Tax:\t\t\t$${quote.tax.toFixed(2)}\n`;
  }
  
  content += `Total:\t\t\t$${quote.total.toFixed(2)}\n\n`;
...
function generateInvoicePdfContent(
  ...
  if (invoice.discount) {
    content += `Discount:\t\t\t$${invoice.discount.toFixed(2)}\n`;
  }
  
  if (invoice.tax) {
    content += `Tax:\t\t\t$${invoice.tax.toFixed(2)}\n`;
  }
