...
test.describe('Survey Scheduling', () => {
  test('should be able to schedule a survey for an accepted quote', async ({ page }) => {
    // Login
    ...
    // Click the Schedule Survey button
    await page.click('button:has-text("Schedule Survey")');
    ...
    // Add notes
    await page.fill('textarea', 'Test survey notes from Playwright test');
    
    // Submit the form
    await page.click('[data-testid="schedule-survey-submit"]');
    
    // Wait for success toast
    await expect(page.locator('.toast')).toContainText('Survey scheduled');
